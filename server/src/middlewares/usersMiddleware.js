...
const validateLogin = async (req, res) => {
    ...
    try {
        ...
        return res.cookie('refresh_token', refreshToken, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 5 * 60 * 1000 }).status(200).json({ message: 'Succesfully authentication', token })
...
const refreshToken = (req, res) => {
    ...
    return res.cookie('refresh_token', refreshToken, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 5 * 60 * 1000 }).status(200).json({ message: 'Succesfully new token', token })
}
...
const deleteRefreshToken = (_req, res) => {
    ...
    return res.clearCookie('refresh_token', { httpOnly: true, secure: true, sameSite: 'strict' }).status(200).json({ message: 'Cookie deleted successfully' })
